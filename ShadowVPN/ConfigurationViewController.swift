...
//        self.configuration["route"] = "chnroutes"
    ...
    func save() {
        updateConfiguration()
        if let result = ConfigurationValidator.validate(configuration: self.configuration as [String : AnyObject]) {
    ...
    func bindData(textField: UITextField, property: String) {
        let val: AnyObject? = configuration[property] as AnyObject
        if let _val = val {
            textField.text = _val as? String
        }
